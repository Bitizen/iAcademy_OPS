...
class Administrator_Controller extends MY_Controller {
	...
	function viewEmployers() {
		...
		$this->data['companyList'] = $this->employer_model->getAllCompanyNames();
		$this->load->helper('form'); */
	...
	function viewEmployer() {
        ...
        $data['myEmployerAffiliatedStudents'] = $this->employer_model->viewAffiliatedStudents();
	...
	function uploadInternResume() {

		$this->load->helper('url');
		$this->load->model('intern_model');
		$student = $this->intern_model->viewIntern();
		
		$config['file_name'] = $student->lastName.'_'.$student->firstName.'_Resume';
		...
		if ( ! $this->upload->do_upload()) {
			$data['uploadErrorResume'] = $this->upload->display_errors();
			...
			$sql = "CALL updateStudentResumePath(?,?)";

  			$student = $this->input->get('sID', TRUE);
	...
	function viewResume(){
		...
		$rPath = $student->resumePath;
		$fName = substr($rPath, 15);
