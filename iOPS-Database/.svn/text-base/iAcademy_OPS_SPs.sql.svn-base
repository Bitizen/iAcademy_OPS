...
--
-- BEGIN EDITS (tied to audit_log triggers)
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `editLandline`(
	IN `myUserID` INT(11)
	, IN `userInput` VARCHAR(60))
BEGIN
		set @user_id = myUserID;
		UPDATE 
			iOPS.users
		SET 
			users.landline = userInput
		WHERE 
			users.id = myUserID;
		set @user_id = null;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `editMobile`(
	IN `myUserID` INT(11)
	, IN `userInput` VARCHAR(60))
BEGIN
		set @user_id = myUserID;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `editEmail`(
	IN `myUserID` INT(11)
	, IN `userInput` VARCHAR(100))
BEGIN
		set @user_id = myUserID;
		UPDATE 
			iOPS.users
		SET 
			users.email = userInput
		WHERE 
			users.id = myUserID;
		set @user_id = null;
END$$
...
DROP PROCEDURE IF EXISTS `viewMyAffiliatedInterns`$$


CREATE DEFINER=`root`@`localhost` PROCEDURE `viewMyAffiliatedStudents`(IN `pUserName` VARCHAR(25))
    NO SQL
BEGIN
declare _eID int(11);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `viewAffiliatedStudents`(IN `employerId` INT(11))
BEGIN
	SELECT
  iops.students.studentID
		, iops.students.firstName
...
CREATE DEFINER =  `root`@`localhost` PROCEDURE  `updateStudentResumePath` ( IN  `pStudentID` INT( 11 ) , IN `pFilePath` VARCHAR( 255 ) ) 
	NO SQL 
